...
//（ただし，この課題をサーブレットではなく Java クラスで作成する場合は，第四引数は不要です）。 
   void kake(int num,int x,boolean a,PrintWriter sc){
//・処理内容 
//　第一引数と第二引数を掛け算し，第三引数の値が true であるときにかぎって，掛け算の結果をさらに２乗します。
//また，この計算結果の値を画面に表示処理します。     
       sc.print(num*x);
   } 
...
//　考えてみましょう。 
//・処理内容 
//　１つめのユーザー定義メソッドの呼び出しを行います。このとき，１つめのユーザー定義メソッドに対して，
//第一引数に対して 5 を，第三引数に false を，それぞれデフォルト値として与えます。 
    void kake(PrintWriter sc){
       kake(5,x,false,sc);
    }
//メソッドを作成したら，それぞれのメソッドに適当な引数を与えて呼び出して画面表示を行い，
